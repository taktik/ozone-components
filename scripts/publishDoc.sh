...
gsutil -h "Cache-Control:no-cache,max-age=0" -m cp -r -J demo/lerna.json $gs_path/lerna.json
gsutil -h "Cache-Control:no-cache,max-age=0" -m cp -r -J demo/lerna.json $latest_path/lerna.json
